...
STATIC_VERSION = "20260827-tracecraft4"
...
async def _lifespan(_app: FastAPI) -> AsyncIterator[None]:
    ...
    if recovered:
        print(
            f"[tracecraft] 已标记 {len(recovered)} 个被中断的项目为可重试："
            f"{', '.join(recovered)}"
        )
    yield


app = FastAPI(title="Tracecraft Web", lifespan=_lifespan)

AUTH_COOKIE = "tracecraft_token"
...
def _read_display_artifact(
    ...
    *,
    ...
    if not text:
        return text
    if key == "analysis":
        # 拆分融合式引用，并在有证据库上下文时把 Agent4 只写的短引用展开为标准
        # [src:...] 引用，避免内部 ID 原样显示在「深度分析」页。
...
async def api_test_search_config() -> dict[str, Any]:
    ...
    result = await web_search("Tracecraft search connectivity check", num_results=1)
...
def main() -> None:
    ...
    if config.WEB_AUTH_TOKEN:
        ...
        print(
            f"[tracecraft] 警告：正在无认证暴露到 {args.host}，"
