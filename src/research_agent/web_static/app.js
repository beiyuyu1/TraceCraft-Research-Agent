...
const state = { projects: [], projectId: Tracecraft.selectedProject(), project: null, artifactKey: null, renderedArtifactKey: null, renderedArtifactMarkup: null, renderedArtifactVersion: null, events: null, busy: false, _artifactTabsBound: false };
...
function renderPipeline(project) {
  ...
  $("pipeline").innerHTML = pipelineStages.map(([label], index) => {
    ...
    return `<div class="pipeline-step ${status}"><span class="step-node">${index < active ? Tracecraft.icon("check", 15) : index + 1}</span><strong>${label}</strong><small>${caption}</small></div>`;
  }).join("");
}
...
function renderTimeline(project) {
  ...
  $("timeline").innerHTML = logs.map((log, index) => `<div class="timeline-row"><time>${Tracecraft.escapeHtml(log.time)}</time><p>${Tracecraft.escapeHtml(log.message)}</p><span class="status-pill ${index === 0 && project.running ? "" : "success"}"><i class="status-dot ${index === 0 && project.running ? "running" : "success"}"></i>${index === 0 && project.running ? "运行中" : "已记录"}</span></div>`).join("");
  $("logList").innerHTML = (project.logs || []).slice().reverse().map((log) => `<div class="log-row"><time>${Tracecraft.escapeHtml(log.time)}</time><span>${Tracecraft.escapeHtml(log.message)}</span></div>`).join("");
}
...
function renderClarification(project) {
  ...
  $("clarifyQuestions").innerHTML = questions.map((question, index) => `
    <label class="clarify-item">
      <span>${index + 1}. ${Tracecraft.escapeHtml(question)}</span>
  ...
  $("clarifyHistory").innerHTML = history.length
    ? `<details style="margin-top:14px"><summary class="muted" style="cursor:pointer;font-weight:700">历史澄清问答（${history.length}）</summary><div class="clarify-history">${history.map((item) => `<div><strong>${Tracecraft.escapeHtml(item.question)}</strong><p>${Tracecraft.escapeHtml(item.answer)}</p></div>`).join("")}</div></details>`
    : "";
}
...
function renderFailure(project) {
  ...
  if (!failed) return;
  $("failureStage").textContent = Tracecraft.stageLabel(project.failed_stage || project.stage);
  ...
  $("failureReasons").innerHTML = reasons.length
    ? `<p class="eyebrow" style="margin-top:14px">${reasonsTitle}</p><ul class="failure-reasons">${reasons.map((item) => `<li>${Tracecraft.escapeHtml(item)}</li>`).join("")}</ul>`
    : "";
...
function renderRerun(project) {
  ...
  select.innerHTML = options.map((item) =>
    `<option value="${Tracecraft.escapeHtml(item.value)}">${Tracecraft.escapeHtml(item.label)}</option>`
  ).join("");
...
function renderResearchPlan(project) {
  ...
  const rows = requirements.map((item) => {
    ...
    return `<tr>
      <td><code>${Tracecraft.escapeHtml(item.question_id)}</code></td>
      <td>${Tracecraft.escapeHtml(item.text)}</td>
      <td>${item.required ? "必答" : "可选"}</td>
      <td>${item.min_supported}</td>
      <td>${Tracecraft.escapeHtml(item.min_source_tier || "不限")}</td>
      <td>${item.require_numeric ? "是" : "否"}</td>
      <td><span class="status-pill ${tone}">${label}</span></td>
    </tr>`;
  }).join("");
  const warning = plan.warning
    ? `<p class="muted">提示：${Tracecraft.escapeHtml(plan.warning)}</p>`
    : "";
...
function renderArtifacts(project) {
  ...
  $("artifactTabs").innerHTML = artifacts.map((artifact) => `<button class="artifact-tab${artifact.key === state.artifactKey ? " active" : ""}${artifact.exists ? "" : " missing"}" type="button" data-artifact="${artifact.key}">${Tracecraft.icon("file", 15)}<span>${Tracecraft.escapeHtml(artifact.label)}</span></button>`).join("");
...
async function loadArtifact(forceTop = false) {
  ...
  try {
    ...
    const artifact = await Tracecraft.api(`/api/projects/${encodeURIComponent(state.projectId)}/artifacts/${encodeURIComponent(artifactKey)}`);
    ...
    const markup = artifact.html || (isJsonArtifact
      ? Tracecraft.renderArtifact(artifactKey, artifact.content)
      : await Tracecraft.renderMarkdownAsync(artifact.content));
    if (state.renderedArtifactKey !== artifactKey || state.renderedArtifactMarkup !== markup) {
      view.innerHTML = markup;
      Tracecraft.hydrateSourceCitations(view, state.projectId);
    ...
    view.innerHTML = `<div class="empty"><span class="empty-symbol">!</span><strong>内容读取失败</strong><p>${Tracecraft.escapeHtml(error.message)}</p></div>`;
  }
}
...
function renderProject(project) {
  ...
  $("stageText").textContent = Tracecraft.stageLabel(project.stage);
...
async function loadProject() {
  ...
  try {
    const project = await Tracecraft.api(`/api/projects/${encodeURIComponent(state.projectId)}`);
    renderProject(project);
    await loadArtifact();
  } catch (error) {
    $("workspaceEmpty").innerHTML = `<span class="empty-symbol">!</span><strong>项目加载失败</strong><p>${Tracecraft.escapeHtml(error.message)}</p><a class="button secondary" href="/app/research">返回研究首页</a>`;
...
async function continueProject() {
  ...
  Tracecraft.setButtonBusy(button, true, "启动中");
  try {
    await Tracecraft.api(`/api/projects/${encodeURIComponent(state.projectId)}/continue`, { method: "POST" });
    Tracecraft.toast("研究已继续运行");
    await loadProject();
...
async function approve(approved) {
  ...
  Tracecraft.setButtonBusy(button, true, approved ? "提交中" : "驳回中");
  try {
    await Tracecraft.api(`/api/projects/${encodeURIComponent(state.projectId)}/approval`, { method: "POST", body: JSON.stringify({ approved, feedback: $("feedbackInput").value }) });
    $("feedbackInput").value = "";
    Tracecraft.toast(approved ? "已通过，研究继续运行" : "已驳回，准备重新执行", approved ? "success" : "warning");
    await loadProject();
  } catch (error) { Tracecraft.toast(error.message, "danger"); }
  finally { Tracecraft.setButtonBusy(button, false); }
}
...
async function typeset() {
  ...
  Tracecraft.setButtonBusy(button, true, "排版中");
  try {
    const result = await Tracecraft.api(`/api/projects/${encodeURIComponent(state.projectId)}/typeset/final-report`, { method: "POST" });
    Tracecraft.toast(result.message, result.status === "pdf" ? "success" : "warning");
    await loadProject();
  } catch (error) { Tracecraft.toast(error.message, "danger"); }
  finally { Tracecraft.setButtonBusy(button, false); }
}
...
async function retryProject(button) {
  state.busy = true;
  Tracecraft.setButtonBusy(button, true, "重试中");
  try {
    const result = await Tracecraft.api(`/api/projects/${encodeURIComponent(state.projectId)}/retry`, { method: "POST", body: JSON.stringify({ extra_rounds: 1 }) });
...
async function migrateResearchPlan(button) {
  state.busy = true;
  Tracecraft.setButtonBusy(button, true, "生成中");
  try {
    const result = await Tracecraft.api(`/api/projects/${encodeURIComponent(state.projectId)}/research-plan/migrate`, { method: "POST" });
    Tracecraft.toast(result.message || "研究需求清单已生成");
  } catch (error) { Tracecraft.toast(error.message, "danger"); }
  finally {
    Tracecraft.setButtonBusy(button, false);
    state.busy = false;
...
async function rerunProject() {
  ...
  Tracecraft.setButtonBusy(button, true, "回退中");
  try {
    const result = await Tracecraft.api(`/api/projects/${encodeURIComponent(state.projectId)}/rerun`, {
      method: "POST",
      body: JSON.stringify({ stage }),
    });
    Tracecraft.toast(result.message || "已回退并重新开始运行", "success");
  } catch (error) { Tracecraft.toast(error.message, "danger"); }
  finally {
    Tracecraft.setButtonBusy(button, false);
    state.busy = false;
...
async function deleteProject(button) {
  ...
  if (!confirmed) { state.busy = false; return; }
  Tracecraft.setButtonBusy(button, true, "删除中");
  try {
    await Tracecraft.api(`/api/projects/${encodeURIComponent(state.projectId)}`, { method: "DELETE" });
    if (state.events) state.events.close();
    localStorage.removeItem("tracecraft.project");
    Tracecraft.toast("项目已删除", "warning");
    if (window.Tracecraft?.navigate) Tracecraft.navigate("/app/research");
...
async function submitClarification(skip) {
  ...
  state.busy = true;
  Tracecraft.setButtonBusy(button, true, "提交中");
  try {
    await Tracecraft.api(`/api/projects/${encodeURIComponent(state.projectId)}/clarification`, {
      method: "POST",
      body: JSON.stringify({ answers, skip }),
    });
    Tracecraft.toast(skip ? "已跳过，Agent1 将使用默认值" : "回答已提交，Agent1 继续起草提纲");
  } catch (error) { Tracecraft.toast(error.message, "danger"); }
  finally {
    Tracecraft.setButtonBusy(button, false);
    state.busy = false;
...
if (window.Tracecraft?.views?.register) {
  Tracecraft.views.register("workspace", { init, destroy });
}
if (!IS_SPA) {
  Tracecraft.mountShell("workspace");
  init();
}
})();
