const Tracecraft = (() => {
  ...
  function sidebar(active) {
    ...
    return `
      <aside class="sidebar">
        <a class="brand" href="/" aria-label="研迹首页">
          <span class="brand-mark" aria-hidden="true"><svg viewBox="0 0 40 40" fill="none"><path d="M7 8h26M20 8v25M12 33l8-8 8 8"/><circle cx="20" cy="21" r="3.5"/></svg></span>
          <span><strong>研迹</strong><small>TRACECRAFT</small></span>
        </a>
  ...
  // ── 视图注册机制（SPA）─────────────────────────────────────
  // 每个视图模块调用 Tracecraft.views.register(name, { init, destroy })。
  ...
  function renderMarkdown(text) {
    ...
    const inline = (value) => {
      let marker = "TRACECRAFT_MARKDOWN_TOKEN";
  ...
  function selectedProject() {
    return new URLSearchParams(window.location.search).get("project") || localStorage.getItem("tracecraft.project") || "";
  }

  function rememberProject(id) {
    if (id) localStorage.setItem("tracecraft.project", id);
  }
...
// 通过 window.Tracecraft 访问（router.js 的 navigate、各视图的 register 判断都依赖它）。
window.Tracecraft = Tracecraft;
