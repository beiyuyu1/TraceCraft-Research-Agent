...
function renderStatus() {
  if (!config) return;
  $("configStatus").innerHTML = `<div class="detail-row"><span>模型 API</span><strong><i class="status-dot ${config.has_api_key ? "success" : "warning"}"></i> ${config.has_api_key ? "已配置" : "缺少 Key"}</strong></div><div class="detail-row"><span>搜索服务</span><strong><i class="status-dot success"></i> ${Tracecraft.escapeHtml(config.search_provider || "anysearch")}</strong></div><div class="detail-row"><span>语义检索</span><strong><i class="status-dot ${config.embedding_model ? "success" : "neutral"}"></i> ${config.embedding_model ? "已启用" : "未启用"}</strong></div>`;
}
...
async function saveWorkspaceConfig() {
  ...
  try {
    ...
    config = await Tracecraft.api("/api/config/workspace", { method: "PUT", body: JSON.stringify(payload) });
    populateConfig();
    Tracecraft.toast(config.source_restart_required ? "设置已保存；材料目录将在重启服务后生效" : "调研与存储设置已保存");
  } catch (error) {
...
async function loadConfig() {
  try {
    config = await Tracecraft.api("/api/config");
    populateConfig();
  } catch (error) {
    $("configStatus").innerHTML = `<div class="warning-box">配置读取失败：${Tracecraft.escapeHtml(error.message)}</div>`;
  }
}
...
async function saveModelConfig() {
  ...
  try {
    ...
    Tracecraft.setButtonBusy(button, true, "保存中");
    config = await Tracecraft.api("/api/config/model", { method: "PUT", body: JSON.stringify(payload) });
    populateConfig();
    Tracecraft.toast("模型配置已保存并生效");
  } catch (error) {
    Tracecraft.toast(error.message, "danger");
  } finally {
    Tracecraft.setButtonBusy(button, false);
...
async function testConnection() {
  ...
  try {
    ...
    Tracecraft.setButtonBusy(button, true, "测试中");
    const result = await Tracecraft.api("/api/config/model/test", { method: "POST", body: JSON.stringify(payload) });
    Tracecraft.toast(result.message || "连接成功");
  } catch (error) {
    Tracecraft.toast(error.message, "danger");
  } finally {
    Tracecraft.setButtonBusy(button, false);
...
async function saveSearchConfig() {
  ...
  try {
    ...
    Tracecraft.setButtonBusy(button, true, "保存中");
    config = await Tracecraft.api("/api/config/search", {
      method: "PUT",
      body: JSON.stringify({ provider, api_key: apiKey || null }),
    });
    populateConfig();
    Tracecraft.toast("搜索配置已保存并生效");
  } catch (error) {
    Tracecraft.toast(error.message, "danger");
  } finally {
    Tracecraft.setButtonBusy(button, false);
...
async function testSearch() {
  ...
  Tracecraft.setButtonBusy(button, true, "测试中");
  try {
    const result = await Tracecraft.api("/api/config/search/test", { method: "POST" });
    Tracecraft.toast(`${result.provider}：${result.message}`);
  } catch (error) {
    Tracecraft.toast(error.message, "danger");
  } finally {
    Tracecraft.setButtonBusy(button, false);
...
function init() {
  ...
  $("copyEnv").addEventListener("click", async () => {
    ...
    try { await navigator.clipboard.writeText(template); Tracecraft.toast("配置模板已复制"); }
    catch (_) { Tracecraft.toast("浏览器不允许复制，请直接编辑项目中的 .env", "warning"); }
  });
  loadConfig();
}
...
if (window.Tracecraft?.views?.register) {
  Tracecraft.views.register("settings", { init, destroy });
}
if (!IS_SPA) {
  Tracecraft.mountShell("settings");
  init();
}
})();
