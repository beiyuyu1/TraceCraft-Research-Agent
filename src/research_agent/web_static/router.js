 ...
 * 视图脚本通过 Tracecraft.views.register(name, {init, destroy}) 注册。
 */
(() => {
  ...
  async function mountView(viewName, { pushState = true } = {}) {
    if (loading) return;
    // 同视图但查询串变化时必须重新挂载，否则 URL 已变、内容仍是旧项目。
    ...
    try {
      const data = cached || await Tracecraft.api(`/api/views/${viewName}`, { signal: currentController.signal });
      if (!cached) fragmentCache.set(viewName, data);
      const mount = $("viewMount");
      if (!mount) return;
      document.title = `${data.title} · 研迹`;
      ...
      currentDef = Tracecraft.views.get(viewName);
      ...
      if (mount) {
        mount.className = "";
        mount.innerHTML = `<div class="empty"><span class="empty-symbol">!</span><strong>视图加载失败</strong><p>${Tracecraft.escapeHtml(error.message)}</p></div>`;
      }
    } finally {
      loading = false;
  ...
  function bootstrap() {
    ...
    Tracecraft.mountShell(activeKey(initial));
  ...
  window.Tracecraft.navigate = function (path) {
